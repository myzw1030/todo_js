...
var dleteFromIncompleteList = function dleteFromIncompleteList(target) {
...
var createIncompleteList = function createIncompleteList(text) {
  ...
  p.className = "text";
  p.innerText = text; // button(完了)タグ生成

  var completeButton = document.createElement("button");
  completeButton.innerText = "完了";
  completeButton.addEventListener("click", function () {
    // 押された完了ボタンの祖先（li）を未完了リストから削除
    dleteFromIncompleteList(completeButton.closest("li")); // 完了リストに追加する要素

    var addTarget = completeButton.closest("li"); // TODO内容テキストを取得

    var text = addTarget.querySelectorAll("p")[0].innerText; // li以下を初期化

    addTarget.textContent = null; // div生成
    ...
    backbutton.addEventListener("click", function () {
      ...
      var text = deleteTarget.querySelectorAll("p")[0].innerText;
      createIncompleteList(text);
    }); // liタグの子要素に各要素を設定

    addTarget.appendChild(div);
    div.appendChild(p);
    div.appendChild(backbutton); // 完了リストに追加する要素
